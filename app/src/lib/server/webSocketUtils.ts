...
export const createWSSGlobalInstance = () => {
    ...
    wss.on('connection', (ws) => {
        ...
        ws.send(JSON.stringify({ event: "currentSocketId", data: ws.socketId }));
        ws.on('message', message => {
            ...
            try {
                ...
                console.error("error on ws parsing", e);
                return;
            }
            console.log("ws message", data);
            if (data.event === "addMetadata") {
                ws.metadata = { ...ws.metadata, ...data.data };
                console.log("ws metadata", ws.metadata);
            }
        });
        ws.on('close', () => {
